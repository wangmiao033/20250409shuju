...
def index():
    df = load_data()
    if df is None:
        return render_template('error.html', message="无法加载数据文件，请检查文件是否存在")
    
    monthly_profit, department_profit = calculate_metrics(df)
    if monthly_profit is None or department_profit is None:
        return render_template('error.html', message="计算数据指标时出错")
    ...
    try:
        ...
        return render_template('error.html', message=f"生成图表时出错: {str(e)}")

@app.route('/error')
def error():
    return render_template('error.html', message="发生未知错误")
